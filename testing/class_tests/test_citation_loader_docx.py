...
class TestCitationLoaderDocx(unittest.TestCase):
    ...
    expected_title_string = "Tehnical Report"
    bibtext_file_location = "../test_documents/bibtextData"
    not_file_type_loader = CitationLoaderDocx(
        bibtext_file_location, regex_location)

    def test_is_docx(self):
        """Asserts the file is .docx
        """
        self.assertTrue(self.loader.is_docx())

    def test_is_not_docx(self):
        """Asserts the file is not .docx
        """
        self.assertFalse(self.not_file_type_loader.is_docx())

    def test_docx_loader_array_size(self):
        """Asserts that the same number of values are extracted each time
        """
        self.assertEqual(83, len(self.citation_array))

    def test_docx_loader_array_consistency(self):
        """Asserts that the ordering of the array is consistent
        """
        self.assertEqual(
            self.citation_array[0].get_title(), self.expected_title_string)

    def test_docx_loader_dict_key_length(self):
        """Asserts that the number of keys in the dict is consistent
        """
        self.assertEqual(1, len(self.citation_dict.keys()))

    def test_docx_loader_dict_key_names(self):
        """Asserts that the key names are as expected
        """

        self.assertEqual(list(self.citation_dict), self.expected_dict_keys)

    def test_docx_loader_dict_values_length(self):
        """Asserts that the number of values in the dict is consistent
        """
        self.assertEqual(
            83, len(self.citation_dict[self.expected_dict_keys[0]]))

    def test_docx_loader_dict_consistency(self):
    ...
    def test_docx_loader_same_length_in_array_and_dict(self):
